<?php
/**
 * Gerir requesições dos elementos outros. 
 *  
...
//die("hello");
 ...
 if( isset($_POST['id_req']) && is_numeric($_POST['id_req']) ){
	...
	if( delRequesicao( $_POST['id_req'], 3 ) > 0 ) 
		die("1");
	else
		die(rawurlencode("De momento não é possível atender ao seu pedido!"));
		
 } else if( isset($_POST['dat_min']) && isset($_POST['dat_lev']) && isset($_POST['id_outro']) 
 && is_numeric($_POST['id_outro'])&& isset($_POST['id_util']) && is_numeric($_POST['id_util'])
 && is_numeric($_POST['sup_out'])&& isset($_POST['sup_out'])
 ){
	
	
	$dat_min = clearGarbadge( $_POST['dat_min'], false, false );
	$dat_lev = cleargarbadge( $_POST['dat_lev'], false, false );
	$id = cleargarbadge( $_POST['id_outro'], false, false );
	$sup_out = cleargarbadge( $_POST['sup_out'], false, false );
	...
	$dat_lev_1[1] , 
	($dat_lev_1[0]+OUTRO_REQ) , 
	$dat_lev_1[2] ) ) < $dat_lev
	) die(rawurlencode("A data do item levantado não pode ser maior do que a data de requesição."));*/
	...
	if( $dat_lev > date("Ymd") || $dat_lev < $dat_min && $dat_lev != "00000000"){
		...
		if( 
		date("Ymd", mktime( 0, 0, 0, $verult[1], $verult[2]+OUTRO_REQ, $verult[0]) )
		> $dat_min
		){
	...
	if( $query ){
		...
		$query = $bd->submitQuery("Select `geral_id_geral`, `outro_nome` From `outro` Where geral_id_geral = (Select `geral_id_geral` From `requesicao` Where `id_requesicao` = $id)");
		
		$query = mysql_fetch_row( $query );
		
		$texto = "
		<b>Olá,</b>
		<p>Informamos que os dados relativos a sua requesição do outro item
		<a href=\"?elem=2&amp;accao=9&amp;outro=$query[0]\">$query[1]</a> foram modificados.</p>
		<i>Com os melhores comprimentos, a gerência.</i>";
		
	
		
		$assunto = "[Outro] $query[1]";
 ...
 $query_text = "
		Select `outro_nome`, `registo_nick`,`registo_id_registo`
		,requesicao.geral_id_geral, id_requesicao
		From `registo`, `requesicao`, `outro` 
		Where registo.id_registo = requesicao.registo_id_registo
		And requesicao.geral_id_geral = outro.geral_id_geral
		";
 ...
 if( isset($_GET['req']) && is_numeric($_GET['req']) ){
	...
	if( existeNaBd( "requesicao", "id_requesicao", $id_req ) == 1 ){
		...
		( $dat_limite[0]+OUTRO_REQ), $dat_limite[2] ) );
		...
		( $expirou["0"]+OUTRO_REQ ), $expirou["2"] ) ) < date("Ymd")) && 
		($q_req_2[0] == "00-00-0000")) ?"Sim":"Não";
		
		$sup_list = listarSupOutro("sup_fil", $id_req, true, false);
		...
		Nome de outro item <input type=\"hidden\" value=\"".$_GET['req']."\" name=\"id_req\" 
		id=\"id_req\" />
		</div>
		</td>
		<td><a href=\"?elem=2&amp;accao=9&amp;outro=$q_req_1[3]\">$q_req_1[0]</a></td>
		</tr>
		...
		<td>
			...
			name=\"apg_req_out\" id=\"apg_req_out\"/>
		</td>
		</tr>
		...
		<td>
			...
			name=\"edit_req_outro\" id=\"edit_req_outro\"/>
		</td>
		</tr>
	...
	if(isset($_GET['psqreqtext']) && isset($_GET['modpsqreq'])){
		...
		switch($_GET['modpsqreq']){
			...
			break;
			case 1: 
			$pesquisar = " And outro.outro_etiqueta Like $pesquisar ";
			break;
			case 2: 
			...
			case 3: 
			$pesquisar 
			= " And DATE_ADD(`requesicao_dat_min`,INTERVAL ".OUTRO_REQ
	...
	DATE_ADD(`requesicao_dat_min`,INTERVAL ".OUTRO_REQ." DAY) < CURDATE() And
	`requesicao_dia_levantado` = '00000000' And `id_elemento` = 3"),0,0);
	...
	`geral_id_geral` = `id_geral` And `id_elemento` = 3"),0,0);
	...
	if($temp == 0)
		$req_exp .= "Não existem requesições de outros itens.";
	else if($temp == 1)
		$req_exp .= "Existe uma requesição de outros itens.";
	else
	...
	<div class=\"ferraopen\">
		...
		<form action=\"?elem=2&accao=4&opcao=3&delallreq=1\" method=\"get\">
			...
			<input type=\"hidden\" value=\"8\"  name=\"accao\" />
			<input type=\"hidden\" value=\"3\"  name=\"opcao\" />
			<input type=\"submit\" value=\"Procurar\" class=\"forms\" />
		</form>
		<a href=\"?elem=2&amp;accao=8&amp;opcao=3\">Listar todas</a><br />
		<a href=\"?elem=2&amp;accao=8&amp;opcao=3&amp;modpsqreq=3&amp;psqreqtext=\">
	...
	while( $req = mysql_fetch_row($query) ){
		...
		echo "<tr><td><a href=\"?elem=2&amp;accao=9&amp;outro=$req[3]\">$req[0]</a></td>
		<td><a href=\"?elem=10&amp;perfil=$req[2]\">$req[1]</a></td>
		<td><a href=\"?elem=2&amp;accao=8&amp;opcao=3&amp;req=$req[4]\">Editar</a></td>
		";
	...
	$bd->submitQuery("Select Count(*) From `requesicao`,`registo`,`outro`
	Where registo.id_registo = requesicao.registo_id_registo And
	requesicao.geral_id_geral = outro.geral_id_geral
	$pesquisar"), 0, 0) ) / 6 );
	...
	if ( $pag_actual > 0 ){
		...
		if(isset($_GET['psqreqtext']) && isset($_GET['modpsqreq'])){
			...
			if(isset($_GET['abranpsq'])){
				...
				<a href=\"?elem=2&amp;accao=8&amp;opcao=3&amp;pagi=" . ( $pagi - 5 ) 
	...
	if ( $query_count_spam > 0 && $pag_actual < $query_count_spam ){
		...
		if(isset($_GET['psqreqtext']) && isset($_GET['modpsqreq'])){
			...
			echo "<div class=\"pags\" style=\"margin-left: 0px;\"><a href=\"?elem=2&amp;accao=8&amp;opcao=3&amp;pagi=" . ( $pagi + 5 ) 
