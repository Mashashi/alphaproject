 ...
 */
 ...
 if( isset($_POST['id_req']) && is_numeric($_POST['id_req']) ){
	...
	if(delRequesicao( $_POST['id_req'] , 1 ) > 0) 
		die("1");
	else
		die(rawurlencode("De momento não é possível atender ao seu pedido!"));
	
 } else if( isset($_POST['dat_min']) && isset($_POST['dat_lev']) && isset($_POST['id_film']) 
 && is_numeric($_POST['id_film'])&& isset($_POST['id_util']) && is_numeric($_POST['id_util'])
 && is_numeric($_POST['sup_fil'])&& isset($_POST['sup_fil'])
 ){
	
	$dat_min = clearGarbadge( $_POST['dat_min'], false, false);
	$dat_lev = cleargarbadge( $_POST['dat_lev'], false, false);
	$id = cleargarbadge( $_POST['id_film'], false, false );
	$sup_fil = cleargarbadge( $_POST['sup_fil'], false, false );
	
	$preg = "#[0-3][0-9]/[0-1][0-9]/2[0-9][0-9][0-9]#";
	
	//Verificar se as datas são válidas
	if( !preg_match($preg, $dat_min) || !preg_match($preg, $dat_lev) && $dat_lev != "00/00/0000"){
		...
		//Verificar se a $data_min ultrapassa em FILME_REQ $dat_lev 
		//echo $dat_min[0].$dat_min[1].$dat_min[2]."<br />".$dat_lev." > ".date("Ymd", mktime(0, 0, 0, $dat_min[1], $dat_min[0]+FILME_REQ, $dat_min[2]) );
		
		if( $dat_lev > date("Ymd", mktime(0, 0, 0, $dat_min[1], $dat_min[0]+FILME_REQ, $dat_min[2]) ) ){
	...
	if( $dat_lev > date("Ymd") || $dat_lev < $dat_min && $dat_lev != "00000000"){
		...
		if( 
		date("Ymd", mktime( 0, 0, 0, $verult[1], $verult[2]+FILME_REQ, $verult[0]) )
		> $dat_min
		){
	...
	`requesicao_id_suporte` = '$sup_fil'
	WHERE `id_requesicao` = $id 
	...
	if( $query ){
		...
		$query = $bd->submitQuery("Select `geral_id_geral`, `filme_nome` From `filme`
		 Where geral_id_geral =
		(Select `geral_id_geral` From `requesicao` Where `id_requesicao` = $id)");
		
		$query = mysql_fetch_row( $query );
		
		$texto = "
		<b>Olá,</b>
		<p>Informamos que os dados relativos a sua requesição do filme 
		<a href=\"?elem=2&amp;accao=5&amp;filme=$query[0]\">$query[1]</a> foram modificados.</p>
		<i>Com os melhores comprimentos, a gerência.</i>";
		
	
		
		$assunto = "[Filme] $query[1]";
 ...
 $query_text = "
		Select `filme_nome`, `registo_nick`,`registo_id_registo`
		,requesicao.geral_id_geral, id_requesicao
		From `registo`, `requesicao`, `filme` 
		Where registo.id_registo = requesicao.registo_id_registo
		And requesicao.geral_id_geral = filme.geral_id_geral
		";
 ...
 if( isset($_GET['req']) && is_numeric($_GET['req']) ){
	...
	if( existeNaBd( "requesicao", "id_requesicao", $id_req ) == 1 ){
		...
		From `requesicao`,`suport_filme` Where `id_requesicao` = $id_req");
		...
		( $dat_limite[0]+FILME_REQ), $dat_limite[2] ) );
		...
		//Se a requesição do filme expirou
		$expirou = explode( "-", $q_req_2[1] );
		
		$expirou = 
		((date( "Ymd", mktime( 0, 0, 0, $expirou["1"], 
		( $expirou["0"]+FILME_REQ ), $expirou["2"] ) ) < date("Ymd")) && 
		($q_req_2[0] == "00-00-0000")) ?"Sim":"Não";
		
		$sup_list = printListSuports("sup_fil", $id_req, true, false);
		...
		Nome de filme <input type=\"hidden\" value=\"".$_GET['req']."\" name=\"id_req\" 
		id=\"id_req\" />
		</div>
		</td>
		<td><a href=\"?elem=2&amp;accao=5&amp;filme=$q_req_1[3]\">$q_req_1[0]</a></td>
		</tr>
	...
	if(isset($_GET['psqreqtext']) && isset($_GET['modpsqreq'])){
		...
		switch($_GET['modpsqreq']){
			...
			break;
			case 1: 
			$pesquisar = " And filme.filme_etiqueta Like $pesquisar ";
			break;
			case 2: 
			...
			case 3: 
			$pesquisar 
			= " And DATE_ADD(`requesicao_dat_min`,INTERVAL ".FILME_REQ
	...
	DATE_ADD(`requesicao_dat_min`,INTERVAL ".FILME_REQ." DAY) < CURDATE() And
	`requesicao_dia_levantado` = '00000000' And `id_elemento` = 1"),0,0);
	...
	From `requesicao` Join `geral` On `geral_id_geral` = `id_geral` And `id_elemento` =1"),0,0);
	
	if($temp == 0)
		$req_exp .= "Não existem requesições de filmes.";
	else if($temp == 1)
		$req_exp .= "Existe a requesição de 1 filme.";
	else
	...
	<div class=\"ferraopen\">
		...
		<form action=\"?elem=2&accao=4&opcao=3&delallreq=1\" method=\"get\">
			...
			<input type=\"hidden\" value=\"4\"  name=\"accao\" />
			<input type=\"hidden\" value=\"3\"  name=\"opcao\" />
			<input type=\"submit\" value=\"Procurar\" class=\"forms\" />
		</form>
		<a href=\"?elem=2&amp;accao=4&amp;opcao=3\">Listar todas</a><br />
		<a href=\"?elem=2&amp;accao=4&amp;opcao=3&amp;modpsqreq=3&amp;psqreqtext=\">
	...
	<tr>
	<td>
	<div class=\"area\" style=\"width: auto;\">Nome do filme</div>
	</td>
	<td>
	<div class=\"area\" style=\"width: auto;\">
	Requerinte
	...
	while( $req = mysql_fetch_row($query) ){
		...
		echo "<tr><td><a href=\"?elem=2&amp;accao=5&amp;filme=$req[3]\">$req[0]</a></td>
		<td><a href=\"?elem=10&amp;perfil=$req[2]\">$req[1]</a></td>
		<td><a href=\"?elem=2&amp;accao=4&amp;opcao=3&amp;req=$req[4]\">Editar</a></td>
	...
	$bd->submitQuery("Select Count(*) From `requesicao`,`registo`,`filme`
	Where registo.id_registo = requesicao.registo_id_registo And
	requesicao.geral_id_geral = filme.geral_id_geral
	$pesquisar"), 0, 0) ) / 6 );
